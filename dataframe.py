...
for idx_participant in range (0,len(participantID_unique)):
    ...
    for idx in range (1,len(date_unique)-1):
        ...
        values = np.sum(actigraph_eachday[:,COL_NUMBERSTEPS])
        number_steps_eachday = np.append(number_steps_eachday ,(values))
        values = np.sum(actigraph_eachday[i,COL_SED])
        sed_eachday = np.append(sed_eachday,(value))
        values = np.sum(actigraph_eachday[i,COL_LIGHT])
        light_eachday = np.append(light_eachday,(value))
        values = np.sum(actigraph_eachday[i,COL_MV])
        mv_eachday = np.append(mv_eachday,(value))
        values = np.sum(actigraph_eachday[i,COL_VIG])
        vig_eachday = np.append(vig_eachday,(value))
    ...
    data_dict['mean_acti_sed'].append(np.mean(number_steps_eachday))
    data_dict['std_acti_sed'].append(np.std(number_steps_eachday))
    data_dict['mean_acti_light_active'].append(np.mean(number_steps_eachday))
    data_dict['std_acti_light_active'].append(np.std(number_steps_eachday))    #for now we can focus on these information from actigraph but we’ll think about other variables to extract from it later
    data_dict['mean_acti_mv_active'].append(np.mean(number_steps_eachday))
    data_dict['std_acti_mv_active'].append(np.std(number_steps_eachday))
    data_dict['mean_acti_vig_active'].append(np.mean(number_steps_eachday))
    data_dict['std_acti_vig_active'].append(np.std(number_steps_eachday))
...
for i in range(110,121):   #No info for dependent variables
    dataframe = dataframe.drop(i,0)
    
X = without_keys(dataframe, varnames)
X = pd.DataFrame.from_dict(X)
#Transforming categorical data
X_gender = pd.get_dummies(X['gender'])
dataframe['gender'] = X_gender
X['gender'] = X_gender
...
corr_threshold = 0.5
dependentVar = abs(cor["mean_steps"])
...
y = dataframe_withA1C["a1c"] #Dependent variable
X = without_keys(dataframe_withA1C, actigraph_variables)
X = pd.DataFrame.from_dict(X) 
#X = X.drop("gender",1)
X_gender = pd.get_dummies(dataframe_withA1C['gender'])
...
X_gender = pd.get_dummies(dataframe_withA1C['gender'])
X_new = pd.concat([Xbe, X_gender['M']], axis=1)
X_woNan = X_new[['mean_steps','mean_morning_steps', 'mean_evening_steps','mean_afternoon_steps','std_steps','std_morning_steps','std_evening_steps','std_afternoon_steps', 'wc','bmi','weight','age','M']]
#X_new = sm.add_constant(Xbe_woNan)
min_max_scaler = preprocessing.MinMaxScaler()
# Create an object to transform the data to fit minmax processor
X_new_scaled = min_max_scaler.fit_transform(X_new)
